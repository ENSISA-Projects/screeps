...
function findRepairTarget(creep) {
  // Trouve les structures endommagées à réparer
  ...
  if (targets.length > 0) {
    // Priorise les structures les plus endommagées
...
function computeReward(creep, action, did, prevState, currentState) {
  ...
  if (action === 'HARVEST') {
    ...
    // Bonus important pour construction
    if (did) reward += 3;
...
function creepState(creep) {
  // state: [hasEnergy, canWork, nearSource, nearTarget, nearConstruction]
  ...
  if (creep.memory.role === 'harvester') {
    ...
    nearConstruction = (constructionSite && creep.pos.getRangeTo(constructionSite) <= 3) ? 1 : 0;
    
    const repairTarget = findRepairTarget(creep);
    nearTarget = (repairTarget && creep.pos.getRangeTo(repairTarget) <= 3) ? 1 : 0;
  }
...
module.exports.run = function(creep) {
  ...
  let doneBuild = false, doneRepair = false;
  ...
  if (actions.includes('BUILD')) doneBuild = usedEnergy === 0 || !findConstructionSite(creep);
  if (actions.includes('REPAIR')) doneRepair = usedEnergy === 0 || !findRepairTarget(creep);
  ...
  if (action === 'HARVEST') {
    ...
    if (usedEnergy > 0 && creep.getActiveBodyparts(WORK) > 0) {
      ...
      if (site) {
        ...
        if (code === OK) did = true;
        else if (code === ERR_NOT_IN_RANGE) creep.moveTo(site, { visualizePathStyle: { stroke: '#00ff00' } });
      }
    }
  }
  else if (action === 'REPAIR') {
    if (usedEnergy > 0 && creep.getActiveBodyparts(WORK) > 0) {
      ...
      if (target) {
        ...
        if (code === OK) did = true;
        else if (code === ERR_NOT_IN_RANGE) creep.moveTo(target, { visualizePathStyle: { stroke: '#0000ff' } });
      }
    }
  }
